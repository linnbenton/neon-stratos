...
// Token mints for Birdeye lookups
...
// Realistic mock price history generator
function mockPriceHistory(basePrice, symbol, points = 24) {
  const volatility = {
    SOL: 0.025, USDC: 0.0002, USDT: 0.0002,
    BTC: 0.018, mSOL: 0.026, JTO: 0.045,
    WIF: 0.065, BONK: 0.07, RAY: 0.04, JUP: 0.035,
  }[symbol] || 0.03
...
const BASE_PRICES = {
  SOL: 148.42, USDC: 1.0, USDT: 1.0, BTC: 63420.1,
  mSOL: 157.73, JTO: 2.84, WIF: 2.18, BONK: 0.0000284,
  RAY: 4.82, JUP: 0.92,
}

const BASE_VOLUMES = {
  SOL: 2_840_000_000, USDC: 8_100_000_000, USDT: 12_300_000_000,
  BTC: 31_200_000_000, mSOL: 24_000_000, JTO: 88_000_000,
  WIF: 420_000_000, BONK: 320_000_000, RAY: 62_000_000, JUP: 140_000_000,
}

function generateMockTokenData(symbol) {
...
async function fetchTokenData(symbol, mint) {
  ...
  try {
    const [overviewRes, historyRes] = await Promise.allSettled([
      fetch(`/api/birdeye/token?mint=${mint}`, { signal: AbortSignal.timeout(6000) }),
      fetch(`/api/birdeye/history?mint=${mint}&type=1H`, { signal: AbortSignal.timeout(6000) }),
    ])
    ...
    const data = { symbol, price, priceChange, volume24h, marketCap, history, isMock: false }
...
export function useBirdeye(tokens = []) {
  ...
  const load = useCallback(async () => {
    ...
    await Promise.allSettled(
      tokens.map(async ({ symbol, mint }) => {
        const d = await fetchTokenData(symbol, mint || KNOWN_TOKENS[symbol] || '')
        results[symbol] = d

...
const MOCK_VAULTS = [
  {
    address: 'USDC-SOL-0',
    tokenA: { mint: 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v', symbol: 'USDC', decimals: 6 },
  ...
  {
    address: 'BTC-SOL-2',
    tokenA: { mint: '9n4nbM75f5Ui33ZbPYXn59EwSgE8CGsHtAeTH5YFeJ9E', symbol: 'BTC',  decimals: 6 },
    tokenB: { mint: 'So11111111111111111111111111111111111111112',    symbol: 'SOL',  decimals: 9 },
    apy: 18.44,
    tvl: 8_330_000,
    volume24h: 1_980_000,
...
const MOCK_POSITIONS = [
  { vault: 'USDC-SOL-0',  tokenA: 'USDC', tokenB: 'SOL',  valueUSD: 12_440,  pnl: 842.3,  pnlPct: 7.27,  shares: 0.0000821, deposited: 11_597.7,  earned: 842.3  },
  { vault: 'SOL-USDT-1',  tokenA: 'SOL',  tokenB: 'USDT', valueUSD: 4_820,   pnl: -128.4, pnlPct: -2.59, shares: 0.0000374, deposited: 4_948.4,   earned: -128.4 },
  { vault: 'JTO-USDC-4',  tokenA: 'JTO',  tokenB: 'USDC', valueUSD: 2_110,   pnl: 418.7,  pnlPct: 24.76, shares: 0.0000501, deposited: 1_691.3,   earned: 418.7  },
  { vault: 'WIF-USDC-5',  tokenA: 'WIF',  tokenB: 'USDC', valueUSD: 940,     pnl: 220.6,  pnlPct: 30.66, shares: 0.0000273, deposited: 719.4,     earned: 220.6  },
]

async function fetchKaminoStrategies() {
  const url = PROXY_API(`${KAMINO_API}/strategies?env=mainnet-beta&status=ACTIVE&strategyType=NON_PEGGED`)
...
function normalizeKaminoVault(raw) {
  ...
  return {
    ...
    priceRange:      [parseFloat(raw.priceLower || 0), parseFloat(raw.priceUpper || 0)],
...
export function useKamino() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setIsMock(true)
      setError(`Live data unavailable — showing simulated vaults. (${err.message})`)
    } finally {
  ...
  return { vaults, positions, loading, error, isMock, lastUpdate, refresh: load }
}

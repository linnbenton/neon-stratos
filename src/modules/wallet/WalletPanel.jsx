...
import { connection, RPC_ENDPOINT } from "../../lib/solana";
...
export default function WalletPanel() {
  ...
  const [debugError, setDebugError] = useState(null); // State baru untuk menangkap error ke UI
  ...
  useEffect(() => {
    ...
    const loadBalance = async () => {
      try {
        setLoading(true);
        setDebugError(null);
        console.log("=== MEMULAI FETCH SALDO ===");
        console.log("Menggunakan RPC:", RPC_ENDPOINT);
        console.log("Address Target:", publicKey.toBase58());

        const lamports = await connection.getBalance(publicKey);
        console.log("Hasil Mentah (Lamports):", lamports);

        setSolBalance(lamports / LAMPORTS_PER_SOL);
      } catch (err) {
        console.error("DETEKSI ERROR WALLET:", err);
        setDebugError(err.message || "Gagal terhubung ke RPC Solana");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="bg-[#0b0f17]/80 backdrop-blur-xl border border-[#1a2332] rounded-2xl p-5 space-y-4">
      ...
      <div>
        ...
        <div className="mt-3">
          ...
          <div className="text-3xl font-bold text-white mt-1 drop-shadow-[0_0_20px_rgba(0,255,163,0.2)]">
            {loading ? "Loading..." : `${solBalance.toFixed(4)} SOL`}
          </div>

          {/* BOX DEBUG ERROR (AKAN MUNCUL DI UI JIKA ADA MASALAH) */}
          {debugError && (

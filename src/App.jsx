...
export default function App() {
  ...
  const allDexes = useMemo(
    () => ["ALL", ...new Set(vaults.map((v) => v.dex))],
    [vaults],
  );
  ...
  const filteredVaults = useMemo(() => {
    return vaults
      .filter((v) => {
        ...
        if (
          q &&
          !`${v.tokenA.symbol}/${v.tokenB.symbol}`.toLowerCase().includes(q)
        )
          return false;
  ...
  return (
    <div className="min-h-screen bg-cyber-bg text-cyber-text flex flex-col grid-line">
      ...
      <main className="flex-1 px-4 md:px-6 py-5 space-y-5 max-w-[1600px] mx-auto w-full">
        ...
        <div className="flex items-center gap-1 border-b border-cyber-border/40">
          {NAV_TABS.map(({ key, label, icon: Icon }) => (
            ...
            >
              ...
              {label}
              {key === "vaults" && (
                <span className="bg-cyber-surface/60 px-1 py-0.5 rounded text-[9px] border border-cyber-border/40">
            ...
            {kaminoLoading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {Array.from({ length: 8 }).map((_, i) => (
                  <div
                    key={i}
                    className="glass-card h-72 animate-pulse bg-cyber-card/40"
                  />
                ))}
              </div>
            ) : filteredVaults.length === 0 ? (
              <div className="glass-card p-12 text-center">
                <div className="font-display text-cyber-dim text-sm">
        ...
        {/* --- MARKET TAB --- */}
        {tab === "market" && <TokenGrid tokenData={tokenData} />}
      </main>
      ...
      <footer className="border-t border-cyber-border/30 px-6 py-3 flex items-center justify-between text-[9px] font-mono text-cyber-dim/50">
        <span>
          KASM // SOLANA DEFI TERMINAL //{" "}
          {vaultsMock ? "SIMULATED DATA" : "LIVE DATA"}
        </span>

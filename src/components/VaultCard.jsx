    ...
    >
      ...
      <div
        className={`h-[2px] w-full bg-gradient-to-r ${
          vault.apy >= 50
            ? "from-cyber-magenta via-cyber-purple to-transparent"
            : vault.apy >= 25
              ? "from-cyber-cyan via-cyber-purple to-transparent"
      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                <span
                  className={`text-[9px] font-mono px-1.5 py-0.5 rounded border ${dexCls}`}
                >
                  {vault.dex}
                </span>
          ...
          <div className="text-right shrink-0">
            <div
              className={`font-display text-xl font-bold ${tier.color} leading-none`}
            >
              {vault.apy.toFixed(2)}%
            </div>
            <div
              className={`text-[9px] font-mono mt-0.5 ${tier.color} opacity-70`}
            >
              APY • {tier.label}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            { sym: symbolB, data: dataB, change: priceChangeB },
          ].map(({ sym, data, change }) => (
            <div
              key={sym}
              className="bg-cyber-surface/30 rounded px-2 py-1.5 flex items-center justify-between gap-1"
            >
              ...
              {data ? (
                <div className="flex items-center gap-1">
                  ...
                  <span
                    className={`text-[9px] font-mono font-bold ${change >= 0 ? "text-cyber-green" : "text-cyber-red"}`}
                  >
                    {change >= 0 ? "▲" : "▼"}
      ...
      {expanded && (
        <div className="border-t border-cyber-border/40 px-4 py-3 bg-cyber-surface/20">
          <div className="grid grid-cols-2 gap-3 text-xs font-mono">
            <div>
              <div className="text-cyber-dim text-[9px] uppercase tracking-widest mb-1">
                Price Range
              </div>
            ...
            <div>
              ...
              <div className="text-cyber-dim text-[10px] truncate">
                {vault.address.slice(0, 14)}…
              </div>
            </div>
          </div>
          ...
          {dataB?.history && (
            <div className="mt-3 rounded overflow-hidden bg-cyber-surface/40">
              <div className="text-[9px] font-mono text-cyber-dim px-2 pt-1.5">
                {symbolB} / USD
              </div>

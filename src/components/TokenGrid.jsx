  ...
  return (
    <div className="glass-card overflow-hidden">
      <div className="px-5 py-3 border-b border-cyber-border/40 flex items-center gap-2">
        ...
        <span className="font-display text-xs font-bold text-cyber-text tracking-widest uppercase">Market Prices</span>
        {selected && (
      ...
      {!active ? (
            ...
            <button key={t.symbol}
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className={`flex items-center gap-0.5 text-[9px] font-mono font-bold
                  ...
                  {t.priceChange >= 0 ? <TrendingUp className="w-2 h-2" /> : <TrendingDown className="w-2 h-2" />}
                  {Math.abs(t.priceChange).toFixed(2)}%
                </span>
              </div>
              <div className="font-mono text-xs font-bold text-cyber-text">{fmtPrice(t.price)}</div>
              <div className="mt-1.5 rounded overflow-hidden h-6">
                <MiniChart data={t.history} symbol={t.symbol} />
              </div>
              <div className="mt-1.5 text-[9px] font-mono text-cyber-dim">{fmtVol(t.volume24h)}</div>
            </button>
          ))}
        </div>
      ) : (
        <div className="p-5">
          <div className="flex items-start justify-between mb-4">
            <div>
              <div className="font-display text-2xl font-bold text-cyber-cyan">{active.symbol}</div>
              <div className="font-display text-3xl font-bold text-cyber-text mt-1">{fmtPrice(active.price)}</div>
              <div className={`flex items-center gap-1.5 mt-2 text-sm font-mono font-bold
                ${active.priceChange >= 0 ? 'text-cyber-green' : 'text-cyber-red'}`}>
                {active.priceChange >= 0 ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
                {active.priceChange >= 0 ? '+' : ''}{active.priceChange.toFixed(4)}%
            ...
            <div className="grid grid-cols-2 gap-3 text-right">
              {[
                ...
                { label: 'MARKET CAP', value: active.marketCap > 0 ? fmtVol(active.marketCap) : '—' },
              ].map(({ label, value }) => (
                <div key={label}>
                  <div className="text-[9px] font-mono text-cyber-dim uppercase tracking-widest">{label}</div>
                  <div className="font-mono text-sm text-cyber-text font-bold mt-0.5">{value}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-3 text-[9px] font-mono text-cyber-dim text-center">
            24H price chart • {active.history.length} data points • {active.isMock ? 'SIMULATED' : 'LIVE'}
          </div>
        </div>
      )}
    </div>
